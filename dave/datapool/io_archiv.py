...
def to_archiv(grid_data):
    ...
    """
    # create Archiv name
    # hier muss noch eine erstellung des archiv namens hin, der eindeutig beschreibt was drin ist
    archiv_name = 'test'
    
    # check if file allready exist
    # hier überprüfen ob schon eine Datei mit dem namen im archiv ordner enthalten ist
    
    # --- create file
    archiv_file = pd.HDFStore(get_data_path(f'{archiv_name}.h5', 'dave_archiv'))
    # area
...
def from_archiv(dataset_name):
    ...
    grid_data.lv_data.lv_nodes = grid_data.lv_data.lv_nodes.append(mv_nodes)
