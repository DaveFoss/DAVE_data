...
def test_sources_availability():
    ...
    datasets = DataFrame({'schema': ['supply',
                                     'supply',
                                     'grid',
                                     'grid',
                                     'grid',
    ...
    for i, dataset in datasets.iterrows():
        request = requests.get(''.join([oep_url, '/api/v0/schema/', dataset.schema, '/tables/',
                                        dataset.table]))
...
def test_oep_request():
    ...
    """
    ehvhv_substations, meta_data = oep_request(schema='grid', table='ego_dp_ehv_substation',
